...
def delete_movie(titles) -> int:
    """
    Remove one or more titles from the cache CSV (case-insensitive exact
    match), so they get re-scraped and re-predicted next time they're
    opened. `titles` can be a single string or a list of strings.
    ...
    mask = df["Title"].str.strip().str.lower().isin(titles_lower)
...
def append_movie(tmdb_data: dict, prediction: dict, notes=None):
    """
    Append one newly predicted movie to the cache CSV.
    `notes` — optional list of data-quality caveat strings (e.g. "Budget
    unknown", "No Reddit posts found") produced by
    feature_engineering.compute_data_notes(). Stored as a single
    "|"-separated string so it round-trips through CSV cleanly.
    """
    _ensure_cache_exists()
    row = {
