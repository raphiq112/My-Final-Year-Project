...
def compute_data_notes(tmdb_data: dict, reddit_data: dict, yt_data: dict,
                        fill_values: dict = None) -> list:
    """
    Flags data-quality caveats that materially affect the success
    classification, so the UI can surface them instead of presenting an
    imputed/proxy-based prediction with the same apparent confidence as a
    fully-observed one. Mirrors the exact fallback rules used in
    engineer_row() and classify_success() above — if either of those
    ...
    if budget <= 0:
        notes.append(
            "Budget not reported by TMDB — financial success was judged against "
            "a flat $100M revenue proxy instead of the usual 2.5\u00d7 budget rule."
        )

    post_count = reddit_data.get('post_count') or 0
    if post_count <= 0:
        notes.append(
            "No pre-release Reddit posts found — Reddit sentiment features were "
            "neutral-filled rather than reflecting real audience discussion."
        )

    comment_count = yt_data.get('comment_count') or 0
    if not yt_data.get('video_id'):
        notes.append(
            "No official trailer could be matched on YouTube — trailer engagement "
            "and comment-sentiment features were neutral/median-filled."
        )
    elif comment_count <= 0:
        notes.append(
            "Trailer found, but no pre-release comments were retrieved (comments "
            "may be disabled) — YouTube comment-sentiment features were neutral-filled."
        )

    trailer_score = yt_data.get('trailer_score')
    if fill_values and (trailer_score is None or trailer_score == 0):
        notes.append(
            "Trailer match confidence score was unavailable — the training-set "
            "median was substituted."
        )
